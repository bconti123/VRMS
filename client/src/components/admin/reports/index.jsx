...
const LocationTableReport = ({
  ...
  prepareDataForReport(eventTypeStats, eventTypes, dataForAllEventsReport);

  prepareDataForReport(hackNightTypeStats, hackNightTypes, dataForHackNightReport);
  ...
  function sortEventsByDate(processedEvents) {
    ...
    sortedEventsByDate = processedEvents.sort(
    ...
    <div className="table-report-wrap">
      {!isLoading ? (
        <div className="admin-table-report">
          {isFilterButton && (
            <button className="filter-button" type="button" onClick={() => handleSetFilterBtn()}>
              Set Filter
            </button>
          )}
                ...
                <tbody>
                  {dataForAllEventsReport.map((event) => (
                    <tr key={`events-${event.location}`}>
                      ...
                      <td key={`h-${event.totalVolunteerHours + event.id}`}>
                        {event.totalVolunteerHours}
                      </td>
                      <td key={`ha-${event.totalVolunteerAvgHours + event.id}`}>
                  ...
                  {isStatsByHackNight &&
                    dataForHackNightReport.map((event) => (
                      <tr key={`hack-night-${event.location}`}>
                        <td key={`${event.location + event.id}`}>{event.location}</td>
                        <td key={`tv-${event.totalVolunteers + event.id}`}>
                          {event.totalVolunteers}
                        </td>
                        <td key={`th-${event.totalVolunteerHours + event.id}`}>
                          {event.totalVolunteerHours}
                        </td>
                        <td key={`ah-${event.totalVolunteerAvgHours + event.id}`}>
                          {event.totalVolunteerAvgHours}
                        </td>
                      </tr>
                    ))}
