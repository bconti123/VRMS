...
const AttendeeTable = ({ attendees, activeMeeting, projectId, setRoster, roster }) => {
  ...
  console.log(
    'roster',
    roster.find((person) => person.userId.name.lastName === 'test'),
  );
    ...
    function sortAttendees() {
      ...
      attendees.forEach((attendee) => {
        ...
        const isOnRoster = Boolean(
          roster.find((teamMember) => teamMember.userId._id === attendee.userId._id),
        );

        currAttendee.isOnRoster = isOnRoster;
        isOnRoster ? attendeesOnRoster.push(currAttendee) : attendeesNotOnRoster.push(currAttendee);
      });
      ...
      {sortedAttendees &&
        sortedAttendees.notOnTeam.map((attendee, index) => {
          return (
            <AttendeeTableRow
              key={Math.random()}
              name={attendee.userId.name.firstName + ' ' + attendee.userId.name.lastName}
